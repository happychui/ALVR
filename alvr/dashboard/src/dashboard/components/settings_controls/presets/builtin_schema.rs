...
pub fn encoder_quality_schema() -> PresetSchemaNode {
    PresetSchemaNode::HigherOrderChoice(HigherOrderChoiceSchema {
        name: "encoder_quality".into(),
        strings: HashMap::new(),
        flags: ["steamvr-restart".into()].into_iter().collect(),
        options: [
            ("Speed", "Speed", "P1"),
        ...
        .map(|(key, val_amd, val_nv)| HigherOrderChoiceOption {
            ...
            modifiers: [
                string_modifier(
                    "session_settings.video.advanced_codec_options.nvenc_overrides.nvenc_quality_preset",
                    val_nv,
                ),
                string_modifier(
                    "session_settings.video.advanced_codec_options.amf_controls.amd_encoder_quality_preset",
                    val_amd,
                ),
            ]
            .into_iter()
